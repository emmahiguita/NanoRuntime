...
const realCommands = {
  ...
  'uname', 'hostname', 'uptime', 'date', 'cal', 'dmesg', 'watch',
...
class StubProc { String name = '?'; int pid = 0; String state = 'S'; }
typedef StubOut = void Function(String, int);

class ProcessManager {
  final List<StubProc> procs = [];
  void ps(StubOut o) { o('ps: rootfs no instalado', Ln.stderr.index); }
  void kill(List<String> a, StubOut o) { o('kill: rootfs no instalado', Ln.stderr.index); }
  void htop(StubOut o) { o('htop: rootfs no instalado', Ln.stderr.index); }
  void pstree(StubOut o) { o('pstree: rootfs no instalado', Ln.stderr.index); }
}

class StubPkg { bool installed = false; String name = ''; }
class PackageRegistry {
  final List<StubPkg> pkgs = [];
  void pkg(List<String> a, StubOut o, [void Function(Duration, void Function())? af]) {
    o('pkg: rootfs no instalado. Ejecuta "bootstrap" primero.', Ln.stderr.index);
  }
}

class StubContainer { String id = ''; String image = ''; String status = 'Exited'; }
...
class PluginRegistry {
  ...
  void plugin(List<String> a, StubOut o) {
    o('plugin: no hay gestor de plugins activo (simulación eliminada).', Ln.stderr.index);
  }
}
class StubPlugin { bool enabled = false; String name = ''; }
...
class TerminalCtx {
  ...
  final Map<String, String> env = {
    'HOME': '/home/nanoai', 'USER': 'nanoai',
    'PATH': '/usr/bin:/bin:/system/bin:/system/xbin',
    'SHELL': '/bin/nanosh', 'LANG': 'en_US.UTF-8',
    'ANDROID_ROOT': '/system', 'ANDROID_DATA': '/data',
  };
  final Map<String, String> aliases = {
    'll': 'ls -la', 'gs': 'git status', 'gp': 'git push', '..': 'cd ..',
  };
