...
void main() {
  ...
  testWidgets('DOOM STRESS: el chat no crashea bajo carga máxima', (
    tester,
  ) async {
    ...
    for (var i = 0; i < 30; i++) {
      ...
      if (st.connection == ModelConnectionState.ready ||
          st.connection == ModelConnectionState.error) {
        break;
      }
    }
    expect(
    ...
    expect(
      container.read(chatProvider).generating,
      isTrue,
    ...
    debugPrint(
      'RAFAGA: generating=${trasRafaga.generating} users=${trasRafaga.messages.where((m) => m.sender == MessageSender.user).length} ais=${trasRafaga.messages.where((m) => m.sender == MessageSender.ai).length} conn=${trasRafaga.connection}',
    );
    expect(trasRafaga.generating, isFalse);
    expect(
      trasRafaga.messages.where((m) => m.sender == MessageSender.user).length,
      1,
      reason:
          'solo 1 mensaje de usuario real: los otros fueron absorbidos por el guard',
    );
    ...
    expect(
      container.read(chatProvider).generating,
      isTrue,
      reason: 'generando antes del STOP',
    );
    chat.stop();
    ...
    expect(
      container.read(chatProvider).generating,
      isFalse,
      reason: 'STOP corta la generación',
    );
    ...
    ); // deja correr el timer del check
    final trasSwitch = container.read(chatProvider);
    expect(
      trasSwitch.activeModel,
      'DeepSeek-R1-7B',
      reason: 'modelo conmutado',
    );
    expect(
      trasSwitch.connection,
      anyOf(ModelConnectionState.ready, ModelConnectionState.error),
    ...
    for (var i = 0; i < 30; i++) {
      await tester.pump(const Duration(seconds: 1));
      if (container.read(chatProvider).connection !=
    ...
    expect(
      ...
      isEmpty,
      reason: 'clear vacía la conversación',
    );
    ...
    debugPrint(
      'HOSTIL: input=${hostil.input.length} users=${hostil.messages.where((m) => m.sender == MessageSender.user).length} ais=${hostil.messages.where((m) => m.sender == MessageSender.ai).length}',
    );
    ...
    expect(
      container.read(chatProvider).generating,
      isFalse,
      reason: 'no queda generación colgada',
    );
    expect(
      container.read(chatProvider).engineOnline,
      isTrue,
      reason: 'motor sano al final',
    );
    expect(
      erroresUi,
      isEmpty,
      reason: 'ningún error de UI capturado durante el stress',
    );

    debugPrint(
      'STRESS OK: ráfaga=${trasRafaga.messages.length} | '
