...
/// Ejecutor fake: devuelve resultados en secuencia (cíclico para reintentos).
...
void main() {
  ...
  test('fallo transitorio (gestureFailed) reintenta hasta éxito', () async {
    final executor = _FakeExecutor(
      tapResults: const [
        AgentExecutionResult.failure(
    ...
    expect(result.completed, isTrue);
    expect(result.steps.single.attempts, 2, reason: 'reintentó 1 vez');
    expect(executor.tapCalls, 2);
  });

  test('postcondición no cumplida reintenta y agota intentos', () async {
    final executor = _FakeExecutor(
      tapResults: const [
        AgentExecutionResult.ok(),
        AgentExecutionResult.ok(),
