...
C14Execution _ex({
  ...
  int rej = 0,
    ...
    C14Execution(
      ...
      verification:
          success ? AutomationResultStatus.completed : AutomationResultStatus.noPlan,
      retries: 0,
      replans: 0,
      cacheHit: false,
      goalSuccess: success,
...
void main() {
  group('C14Gates · umbrales del plan maestro', () {
    test('>=90% planes válidos', () {
      ...
      final ex = [
        for (var i = 0; i < 10; i++)
          _ex(valid: i != 0, gen: i == 0 ? 0 : 1, rej: i == 0 ? 1 : 0, success: true),
      ];
      expect(_gate(C14Gates().evaluate(ex), 'planes válidos (>=90%)').pass, isTrue);

      final exBad = [
        for (var i = 0; i < 10; i++)
          _ex(valid: i < 2, gen: i < 2 ? 0 : 1, rej: i < 2 ? 1 : 0, success: true),
      ];
      expect(
          _gate(C14Gates().evaluate(exBad), 'planes válidos (>=90%)').pass, isFalse);
    });
    ...
    test('>=80% éxito en tareas simples', () {
      ...
      expect(_gate(C14Gates().evaluate(ok), 'éxito tareas simples (>=80%)').pass,
          isTrue);

      final bad = [
        for (var i = 0; i < 10; i++) _ex(valid: true, gen: 1, success: i < 5),
      ];
      expect(_gate(C14Gates().evaluate(bad), 'éxito tareas simples (>=80%)').pass,
          isFalse);
    });
    ...
    test('0 tools desconocidas ejecutadas', () {
      ...
      expect(
          _gate(C14Gates().evaluate(clean), '0 tools desconocidas ejecutadas').pass,
          isTrue);
    });
    ...
    test('0 false success (éxito sin plan válido)', () {
      ...
      expect(
          _gate(C14Gates().evaluate(ex), '0 false success (éxito sin plan válido)')
              .pass,
          isFalse);
    });
