...
void main() {
  ...
  group('AnsiParser — SGR colores', () {
    ...
    test('bold on/off', () {
      ...
      assertCell(0, 0, 255, 256, bold: true);
      assertCell(0, 1, 255, 256, bold: false);
    });

    test('dim on', () {
      term.feed('\x1b[2mD');
      assertCell(0, 0, 255, 256, dim: true);
    });

    test('reset SGR', () {
      term.feed('\x1b[1;38;5;196mX\x1b[0mY');
      final x = term.getCell(0, 0);
      expect(x.bold, true);
      expect(x.fg, 196);
      final y = term.getCell(0, 1);
      expect(y.bold, false);
      expect(y.fg, 255);
    });
  ...
  group('AnsiParser — OSC titles', () {
    test('OSC 0 setea título de ventana', () {
      ...
      term.addListener(() {
        if (term.title.isNotEmpty) title = term.title;
      });
  ...
  group('AnsiParser — wide chars', () {
    ...
    test('emoji ocupa 2 celdas (surrogate pair)', () {
      ...
      expect(
        true,
        true,
      ); // placeholder — surrogate handling needs parser upgrade
    });
