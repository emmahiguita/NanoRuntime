...
async fn main() {
    ...
    match test_config_loading() {
        Ok(()) => {
            passed += 1;
            println!("  ✓ Config loads and validates")
        }
        Err(e) => {
            failed += 1;
            println!("  ✗ Config: {}", e)
    ...
    for (name, text, should_detect) in &pii_tests {
        if *name == "Anonymization" {
            ...
            if anon.contains("[EMAIL]")
                && anon.contains("[PHONE]")
                && !anon.contains("juan@mail.com")
                && !anon.contains("555-123-4567")
            {
                passed += 1;
    ...
    if high_conf > 0.5 && low_conf < 0.5 {
        passed += 1;
        println!(
            "  ✓ Confidence scoring: high={:.3}, low={:.3}",
            high_conf, low_conf
        );
    } else {
        failed += 1;
    ...
    match test_tool_executor().await {
        Ok(()) => {
            passed += 1;
            println!("  ✓ Tool registration and listing")
        }
        Err(e) => {
            failed += 1;
            println!("  ✗ Tools: {}", e)
    ...
    match test_tool_validation().await {
        Ok(()) => {
            passed += 1;
            println!("  ✓ Tool parameter validation")
        }
        Err(e) => {
            failed += 1;
            println!("  ✗ Tool validation: {}", e)
        }
    }
    ...
    match test_template_interpolation().await {
        Ok(()) => {
            passed += 1;
            println!("  ✓ Template interpolation ({{{{var}}}} syntax)")
        }
        Err(e) => {
            failed += 1;
            println!("  ✗ Template: {}", e)
    ...
    match test_routing_pii().await {
        Ok(()) => {
            passed += 1;
            println!("  ✓ PII forces local routing")
        }
        Err(e) => {
            failed += 1;
            println!("  ✗ Routing: {}", e)
    ...
    if stats.total_system_mb > 0 && stats.available_mb > 0 {
        passed += 1;
        println!(
            "  ✓ Memory stats: {}MB total, {}MB available",
            stats.total_system_mb, stats.available_mb
        );
    } else {
        failed += 1;
    ...
    if est_1b5 < est_7b && est_7b < est_14b {
        passed += 1;
        println!(
            "  ✓ Model memory estimates: 1.5B={}MB, 7B={}MB, 14B={}MB",
            est_1b5, est_7b, est_14b
        );
    } else {
        failed += 1;
    ...
    match test_grammar() {
        Ok(()) => {
            passed += 1;
            println!("  ✓ JSON tool call grammar")
        }
        Err(e) => {
            failed += 1;
            println!("  ✗ Grammar: {}", e)
    ...
    match test_yes_no_grammar() {
        Ok(()) => {
            passed += 1;
            println!("  ✓ Yes/No grammar validation")
        }
        Err(e) => {
            failed += 1;
            println!("  ✗ Yes/No: {}", e)
...
async fn test_vector_engine() -> Result<(), String> {
    ...
    engine
        .index_document(
            ...
            serde_json::json!({"source": "article"}),
            None,
        )
        .await
        .map_err(|e| e.to_string())?;
    ...
    let results = engine
        .search("NanoAI Rust runtime", 5, None)
        .await
        .map_err(|e| e.to_string())?;
    if results.is_empty() {
...
async fn test_vector_clear() -> Result<(), String> {
    ...
    let engine = VectorEngine::new(&config)
        ...
        .map_err(|e| e.to_string())?;
    engine
        .index_document("test", serde_json::json!({}), None)
        .await
        .map_err(|e| e.to_string())?;
...
fn test_yes_no_grammar() -> Result<(), String> {
    ...
    if !grammar.validate("Yes") {
        return Err("'Yes' rejected".into());
    }
    if !grammar.validate("No") {
        return Err("'No' rejected".into());
    }
    if grammar.validate("Maybe") {
        return Err("'Maybe' accepted".into());
    }
    if grammar.validate("yes") {
        return Err("'yes' accepted (case-sensitive)".into());
    }
    Ok(())
}
