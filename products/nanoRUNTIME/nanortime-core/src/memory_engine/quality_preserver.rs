...
impl QualityPreserver {
    ...
    pub fn measure_perplexity(logits: &[f32], token_ids: &[u32]) -> f32 {
        ...
        for &token_id in token_ids {
            if (token_id as usize) < logits.len() {
                let log_prob = (logits[token_id as usize] - max_logit) as f64 - exp_sum.ln();
                total_nll -= log_prob;
                count += 1;
            }
        }
    ...
    pub fn evaluate(&mut self, current_ppl: f32) -> QualityReport {
        ...
        let (protect_critical, recommended_compression) =
            if quality_drop_pct > self.conservative_threshold {
    ...
    pub fn set_baseline(&mut self, baseline: f32) {
        ...
        tracing::info!(
            "QualityPreserver: baseline perplexity set to {:.2}",
            baseline
        );
    }
...
mod tests {
    ...
    fn test_measure_perplexity_certain() {
        ...
        assert!(
            ppl < 5.0,
            "Certain prediction should have low perplexity: {}",
            ppl
        );
    }
