...
impl AdaptiveScheduler {
    ...
    pub fn schedule(
        ...
        for (layer_id, &score) in attention_scores.iter().enumerate() {
            ...
            let (last_access, access_count, is_critical) = {
                // Safety: if attention_scores has more entries than n_layers,
                // skip the extra entries rather than panicking.
                let p = match self.layer_priorities.get(&layer_id) {
                    Some(p) => p,
                    None => continue, // layer not registered, skip
                };
        ...
        let mut sorted_layers: Vec<(usize, f32, f64)> = self
            .layer_priorities
            .values()
        ...
        let layers_to_prefetch: Vec<usize> = predicted_layers
            .iter()
    ...
    pub fn adjust_strategy(&mut self, quality_drop_pct: f32) {
        ...
        if old_strategy != self.strategy {
            tracing::info!("Strategy changed: {:?} → {:?}", old_strategy, self.strategy);
        }
    }
...
mod tests {
    ...
    fn test_calculate_priority_critical_layer() {
        let p = AdaptiveScheduler::calculate_priority(0, false, 0.5, true, Instant::now(), 10);
    ...
    fn test_calculate_priority_predicted_layer() {
        let p_predicted =
            AdaptiveScheduler::calculate_priority(5, true, 0.5, false, Instant::now(), 5);
        let p_normal =
            AdaptiveScheduler::calculate_priority(5, false, 0.5, false, Instant::now(), 5);
        assert!(
            p_predicted > p_normal,
            "Predicted layers should have higher priority"
        );
    }
    ...
    fn test_top_priority_layers() {
        ...
        assert!(
            has_expected,
            "Top 5 should contain critical or high-access layers, got: {:?}",
            top
        );
    }
}
