...
//!
//! ## Deprecation notice (SOLID consolidation)
//! This module overlaps significantly with `hardware_hal.rs`. The canonical
//! hardware detection API is `hardware_hal::profile_device()`. Use that for
//! new code. `HardwareProfiler` is retained only for its `cached_profile`
//! field — it wraps `hardware_hal` with a caching layer.
//!
//! TODO: merge `benchmark_ssd` (~lines 176-210) into `hardware_hal::bench_storage`
//! which has a more complete sequential I/O benchmark.
...
mod tests {
    ...
    fn test_classify_low_end() {
        assert_eq!(
            HardwareProfiler::classify(4096, 100.0, 2),
            DeviceClass::LowEnd
        );
    }
    ...
    fn test_classify_mid_end() {
        assert_eq!(
            HardwareProfiler::classify(8192, 250.0, 4),
            DeviceClass::MidEnd
        );
    }
    ...
    fn test_classify_high_end() {
        assert_eq!(
            HardwareProfiler::classify(32768, 1000.0, 16),
            DeviceClass::HighEnd
        );
    }
    ...
    fn test_estimate_layers_moe() {
        ...
        assert!(
            layers > layers_dense || layers == 32,
            "MoE should fit more layers than dense"
        );
    }
