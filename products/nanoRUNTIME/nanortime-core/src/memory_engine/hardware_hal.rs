...
pub fn profile_device() -> DeviceProfile {
    ...
    // CPU cores
    p.cpu_cores = std::thread::available_parallelism()
        .map(|n| n.get() as u32)
        .unwrap_or(4);
    ...
    {
        // Query real RAM and storage via sysinfo (works on all platforms)
        use sysinfo::System;
        let sys = System::new_all();
        p.ram_total_mb = sys.total_memory() / 1_048_576; // bytes → MB
        p.ram_available_mb = sys.available_memory() / 1_048_576;
        p.oom_score = -1;
        p.oom_score_adj = 0;
        p.cpu_temp_c = -1;
    }
