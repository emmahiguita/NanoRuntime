...
impl RateLimiter {
    /// Safe lock helper: recovers from poisoned mutex instead of panicking.
    /// Returns a guard or resets the state if poisoned (last resort).
    fn lock_state(&self) -> std::sync::MutexGuard<'_, BucketState> {
        match self.state.lock() {
            Ok(g) => g,
            Err(poisoned) => {
                tracing::error!("RateLimiter Mutex poisoned — resetting state");
                poisoned.into_inner()
            }
        }
    }
