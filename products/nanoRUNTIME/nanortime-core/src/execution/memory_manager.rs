...
pub struct MemoryManager {
    ...
    #[allow(dead_code)]
    gpu_layers: i32,
    /// Memoria real del modelo cargado (MB). Actualizado por ModelManager al cargar.
    model_mb: u64,
...
impl MemoryManager {
    ...
    fn refresh_sys(&self) -> std::sync::MutexGuard<'_, Option<System>> {
        let mut sys_guard = match self.sys.lock() {
            Ok(g) => g,
            Err(poisoned) => {
                tracing::error!("MemoryManager sys Mutex poisoned — recovering");
                poisoned.into_inner()
            }
        };
    ...
    pub fn get_stats(&self) -> MemoryStats {
        ...
        let device_class = {
            let mut profiler_guard = match self.profiler.lock() {
                Ok(g) => g,
                Err(p) => {
                    tracing::error!("MemoryManager profiler Mutex poisoned — recovering");
                    p.into_inner()
                }
            };
    ...
    pub fn set_model_memory(&mut self, model_mb: u64) {
        self.model_mb = model_mb;
        tracing::debug!("Model memory updated: {} MB", model_mb);
    }
    ...
    pub fn recommend_context_size(
        ...
        if ram_for_kv < 50 {
            tracing::warn!(
                "Very little RAM available for KV cache ({}MB). Using 512 context.",
                ram_for_kv
            );
            return 512;
        }
    ...
    pub fn auto_configure_v2(model_file_mb: u64, max_context: u32) -> (u32, u32, String) {
        ...
        tracing::info!(
            ...
            risk,
            streaming,
            plan.estimated_vma_mb,
            plan.estimated_rss_mb
        );

        // V2 risk assessment (replaces oom_guard)
        let vma_ratio = plan.estimated_vma_mb / profile.ram_total_mb as f64;
        let risk_level = if vma_ratio > 0.90 {
            "Critical"
        ...
        if vma_ratio > 0.75 {
            ...
            let survival = planner.plan_survival(
                profile.ram_available_mb as f64,
                profile.ram_total_mb as f64,
                &profile,
            );
            return (
        ...
        (ctx.min(max_context).max(256), batch.clamp(128, 512), risk)
    }
    ...
    pub fn auto_configure(model_file_mb: u64, max_context: u32) -> (u32, u32) {
        ...
        // Batch size: bigger = faster prompt processing.
        // Budget: 128 (survival), MidRange: 256, Flagship: 512, Desktop: 1024
        let batch = if available_mb >= 16384 {
            1024
...
mod tests {
    ...
    fn test_has_enough_memory() {
        ...
        assert!(
            stats.total_system_mb > 0,
            "Should detect real total memory > 0"
        );
        assert!(
            stats.available_mb > 0,
            "Should detect real available memory > 0"
        );
    ...
    fn test_real_memory_detected() {
        ...
        assert!(
            stats.total_system_mb >= 1024,
            "Total memory should be at least 1GB"
        );
        assert!(
            stats.total_system_mb <= 1_000_000,
            "Total memory should be realistic"
        );
    }
