...
impl Router {
    ...
    /// 3. Tier 3 habilitado → Local primero, escalación post-generación
    ///    (Cloud escalation logic lives in Orchestrator::execute_local_with_escalation,
    ///    not in the router — the router always defers to Local and the orchestrator
    ///    evaluates confidence post-generation to decide whether to re-route to Cloud)
    /// 4. Tier 2 habilitado → LAN
    /// 5. Default → Local
    pub async fn route(&self, prompt: &str) -> Result<RoutingDecision> {
        ...
        if self.config.hybrid_routing.enabled && self.config.tiers.tier3.enabled {
            tracing::debug!(
                "Tier 3 cloud available — will escalate post-generation if confidence low"
            );
            return Ok(RoutingDecision::Local);
        }
...
mod tests {
    ...
    async fn test_route_cloud_defers_to_local_first() {
        ...
        let router = Router::new(config);
        let decision = router
            .route("Explica la teoría de la relatividad")
            .await
            .unwrap();
    ...
    async fn test_route_lan_activated_when_tier2_enabled() {
        ...
        if let RoutingDecision::Lan(ep) = decision {
            assert!(
                ep.contains("192.168.1.100"),
                "LAN endpoint should be the configured IP"
            );
        }
    }
    ...
    async fn test_route_lan_pii_still_forces_local() {
        ...
        let decision = router.route("Mi SSN es 123-45-6789").await.unwrap();
        assert!(matches!(decision, RoutingDecision::Local));
    }
