...
bool llama_batch_allocr::init(
    ...
    if (n_pos_per_embd > 1) {
        ...
        for (uint32_t s = 0; s < n_seq_max; ++s) {
            ...
            // MTP hook batches (draft-mtp) carry BOTH token and embd (h_nextn row)
            // and legitimately re-decode the same positions on every draft/verify
            // round-trip: the MTP context runs a single dense attention layer
            // (filtered to il >= n_layer) with no recurrent state, so position
            // monotonicity is not required there. Regular batches never set both.
