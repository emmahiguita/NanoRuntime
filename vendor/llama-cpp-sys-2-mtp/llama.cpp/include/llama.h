...
extern "C" {
    ...
    // NanoRuntime: base address + size (bytes) of the primary mmap of the
    // model file. NULL if the model is not mmap'd. Combine with per-tensor
    // file offsets (GGUF index) to madvise()/prefetch individual layers.
    LLAMA_API void * llama_model_mmap_addr(const struct llama_model * model, size_t * size);
