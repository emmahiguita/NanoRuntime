...
impl LlamaModel {
    ...
    /// Raw `llama_model` pointer. Exposed for NanoRuntime's OS-like layer
    /// streaming (madvise per layer via `llama_model_mmap_addr`).
    ///
    /// # Safety
    /// The pointer is valid while `self` is alive. Callers must not free it.
    #[must_use]
    ...
    /// Combine with per-tensor file offsets (GGUF index) to `madvise` /
    /// prefetch individual layers — NanoRuntime's OS-like layer streaming.
    #[must_use]
    pub fn mmap_addr(&self) -> (*mut std::ffi::c_void, usize) {
        let mut size = 0usize;
        let addr =
            unsafe { llama_cpp_sys_2::llama_model_mmap_addr(self.model.as_ptr(), &mut size) };
        (addr, size)
    }
