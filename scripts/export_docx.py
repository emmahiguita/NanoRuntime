...
add_heading_sub("On-Device LLM Inference")
add_para("MLC-LLM compiles models to native GPU kernels via TVM and targets OpenCL/Vulkan on mobile GPUs. PowerInfer exploits activation sparsity to skip 'cold' neurons, reducing arithmetic operations by up to 45%. Our system targets CPU-only inference on devices without a programmable GPU, a constraint that limits both MLC-LLM and PowerInfer.")

add_heading_sub("Memory Management for Large Models")
add_para("vLLM introduced PagedAttention, a virtual-memory-inspired KV-cache management that dramatically reduces fragmentation. FlexGen coordinates GPU-CPU-NVMe offloading with a linear programming solver to maximize throughput under constrained hardware. LLM in a Flash exploits flash storage bandwidth to run models 2× larger than available RAM by loading only a sparse subset of weights on-demand. Our approach is orthogonal: we target mobile UFS storage with 3-10× lower bandwidth than NVMe SSDs, and we combine OS-level madvise paging with Graceful Degradation to handle the additional constraint of a trigger-happy Android OOM killer that standard offloading strategies do not account for. Collaborative Inference partitions transformer layers between device and cloud based on available bandwidth. Prior routing systems use deterministic rules; we instead use the model's own token-level entropy as a real-time confidence signal, eliminating the need for a separate classifier and making routing task-agnostic.")

# 3. System Design
add_heading_section("3. System Design")
add_heading_sub("3.1 Architecture Overview")
add_para("NanoRuntime is implemented in Rust (~12k lines across three crates: nanortime-core, nanortime-ffi, nanortime-cli) and wraps the llama-cpp-2 Rust binding to the llama.cpp backend. The system operates in three layers:")
add_para("1. Inference Layer (nanortime-ffi): Thin FFI wrapper over llama.cpp providing model loading (NanoModel), context creation (NanoContext), and streaming generation with per-token probability vectors.", size=10)
add_para("2. Orchestration Layer (nanortime-core): Routing, privacy, RAG, prompt caching, and memory management.", size=10)
add_para("3. CLI/FFI Layer: Interactive terminal and Android JNI bridge.", size=10)

add_heading_sub("3.2 OS-Level Dynamic Memory Paging")
add_para("When llama.cpp loads a GGUF file with use_mmap=true, the kernel maps the file into the process's virtual address space but does not immediately populate physical RAM. Pages are faulted in on first access (demand paging). On Android, the OOM killer may evict these pages under memory pressure, causing page faults that stall generation. Our approach makes page residency explicit and predictable.")
add_para("GGUFLayoutAnalyzer parses the GGUF header to determine the byte offset and size of each transformer layer. The analyzer groups contiguous layers into prefetch batches to minimize syscall overhead.")
add_para("OSMemoryPaginator issues madvise(MADV_WILLNEED) for prefetch and madvise(MADV_DONTNEED) for eviction, with page-aligned addresses (4096 bytes). On Windows, PrefetchVirtualMemory and DiscardVirtualMemory are used equivalently.")
add_para("AdaptiveScheduler queries /proc/meminfo (Android) or GlobalMemoryStatusEx (Windows) before each forward pass. If available RAM falls below a configurable watermark (default: 15% of total), the scheduler reduces the KV cache context window by 25% increments until RAM pressure is resolved, or returns ContextTooSmall if the minimum viable context (512 tokens) cannot be satisfied.")

add_heading_sub("3.3 Entropy-Driven Hybrid Routing")
add_para("After each local generation, NanoRuntime computes the normalized Shannon entropy of the token probability sequence: H_norm = -Sum(p_t × log2(p_t)) / log2(|V|), where |V| is the vocabulary size (151,936 for Qwen-2.5). H_norm ranges from 0 to 1; low values indicate high model confidence. The confidence score is c = 1 - H_norm.")
...
add_para("On the OPPO CPH2557, vanilla llama.cpp with --no-mmap triggers an OOM termination when attempting to load the 4.47 GB DeepSeek-7B model. NanoRuntime completes inference successfully with a peak RSS of 4.82 GB, achieving a file-to-RAM ratio of 1.08×. On PC, NanoRuntime achieves 1.84 GB peak RSS vs 2.04 GB for llama.cpp with the 1.07 GB Qwen 1.5B model.")

add_heading_sub("4.4 Cross-Device Scalability Validation")
add_para("To validate scalability across hardware tiers, we extended stress testing to the Samsung Galaxy A30s (Exynos 7904, 3.72 GB total RAM, ~1.43 GB usable at launch). The NanoRuntime binary was deployed without recompilation. On this severely memory-constrained device, Graceful Degradation automatically reduced context from 8,192 to 512 tokens and batch size from 512 to 256, as evidenced by the runtime log:")
add_code("WARN  Very little RAM available for KV cache (0MB). Using 512 context.")
add_code("INFO  Auto-configure: RAM 1426MB avail/3724MB total, ctx=512, batch=256")
add_code("INFO  RAM optimization: reducing context from 8192 to 512, batch from 512 to 256")
add_para("Over 30 consecutive queries on the Samsung A30s, the system maintained 100% success rate (30/30) with zero OOM terminations. Available RAM exhibited a net increase of 351 MB (from 1,834 MB to 2,185 MB over 30 iterations). On the OPPO CPH2557, 50 consecutive queries confirmed this pattern: available RAM increased by 296 MB net (3,665 MB → 3,962 MB, 50/50 success). The consistent upward trend refutes memory leaks; a downward trend would indicate leaks.")
...
add_heading_sub("4.5 PC Ablation: Memory Stability vs Throughput")
add_para("To isolate the effect of our madvise policy from Android-specific variables, we conduct a controlled ablation on PC hardware (Windows 11, 32 GB RAM, NVMe SSD) comparing NanoRuntime against vanilla llama.cpp under two configurations: --no-mmap and --mmap.")
add_table(
...
# 5. Discussion
add_heading_section("5. Discussion")

add_heading_sub("5.1 Limitations")
add_para("The Throughput-Stability Trade-off: Our PC ablation confirms that NanoRuntime's rigorous memory management incurs a throughput penalty (~48% on PC, and an I/O-bound limit of ~0.43 tok/s for 7B models on mobile, versus ~20 tok/s unconstrained). However, in Edge computing, liveness guarantees supersede raw speed. A system that generates 10 tok/s reliably is infinitely more valuable than a system that attempts 20 tok/s but crashes due to OOM.")
add_para("Inference speed: At 0.43 tok/s (7B) and 2.27-3.51 tok/s (1.5B) on Android CPU, NanoRuntime is not suitable for real-time dialogue. GPU offloading is the highest-impact future improvement.")
add_para("RAM lower bound: The system requires approximately 2.5 GB of free RAM for 7B Q4 model weights plus KV-cache overhead. Devices with <4 GB total RAM cannot run 7B models but successfully run 1.5B models with Graceful Degradation.")
add_para("Entropy threshold calibration: The threshold τ = 0.85 was set empirically on our prompt set. For production use, τ should be calibrated per task using a held-out validation set.")
