...
void main() {
  ...
  testWidgets('PTY: bootstrap rootfs + sesión interactiva real', (tester) async {
    ...
    Future<void> run(String cmd, {Duration wait = const Duration(milliseconds: 800)}) async {
      final f = termField();
      await tester.tap(f);
    ...
    debugPrint('PTY-TEST: bootstrap disparado — esperando descarga/extracción...');
    ...
    for (var i = 0; i < 300; i++) {
      ...
      final bashOk = File('/data/user/0/dev.nanoai.mobile/files/nano/usr/bin/bash')
          .existsSync();
      ...
      final alreadyOk = find
          ...
          .evaluate()
          .isNotEmpty;
      if (bashOk || doneOk || alreadyOk) { installed = true; break; }
      if (failOk) break;
      if (i % 15 == 0) {
        debugPrint('PTY-TEST: esperando extracción... ${i}s zip=${File("/data/user/0/dev.nanoai.mobile/files/nano/bootstrap-aarch64.zip").existsSync()} usr=${Directory("/data/user/0/dev.nanoai.mobile/files/nano/usr").existsSync()}');
      }
    }
    debugPrint('PTY-TEST: rootfs instalado=$installed');

    // Inspección del sandbox tras la extracción (diagnóstico del check).
    final sb = '/data/user/0/dev.nanoai.mobile/files/nano';
    debugPrint('PTY-TEST: sandbox inspección:');
    for (final p in ['', '/usr', '/usr/bin', '/usr/bin/bash', '/bin', '/bin/bash', '/SYMLINKS.txt', '/bootstrap-aarch64.zip']) {
      final f = File('$sb$p');
      debugPrint('  $p → exists=${f.existsSync()} size=${f.existsSync() ? f.lengthSync() : -1}');
    }
    // ¿SYMLINKS.txt se procesó? ¿usr/bin tiene links?
    try {
      final usrBin = Directory('$sb/usr/bin').listSync().take(8).toList();
      debugPrint('  usr/bin entries: ${usrBin.map((e) => e.uri.pathSegments.last).join(", ")}');
    } catch (e) {
    ...
    if (!installed) {
      ...
      for (final t in texts) debugPrint('  | $t');
    ...
    final ansiPresent = find.byType(AnsiTerminalView, skipOffstage: false)
        .evaluate()
        .isNotEmpty;
    ...
    expect(find.byType(NanoTerminal), findsWidgets, reason: 'terminal vivo tras PTY');
    debugPrint('PTY-TEST OK: rootfs instalado + bash/python interactivo vía PTY real');
  });
}
