...
void main() {
  ...
  testWidgets('barrido: dashboard, chat, models y settings 100% funcionales', (tester) async {
    ...
    expect(dash.isLive, isTrue,
        reason: 'MethodChannel com.nanoai/device_metrics debe reportar datos reales');
    expect(dash.ramTotalGb, greaterThan(0));
    expect(dash.batteryPct, greaterThanOrEqualTo(0), reason: 'batería real del device');
    ...
    expect(modelsState.models.length, 4, reason: 'catálogo expone 4 modelos reales');
    ...
    for (var i = 0; i < 10; i++) {
      ...
      if (container.read(chatProvider).activeModel == 'DeepSeek-R1-7B' &&
          container.read(chatProvider).connection != ModelConnectionState.loadingModel) break;
    }
