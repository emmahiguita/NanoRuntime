...
INFERENCE_MAX_TOKENS = int(os.environ.get("NANO_MAX_TOKENS", "48"))
INFERENCE_INTERVAL_S = float(os.environ.get("NANO_INFERENCE_INTERVAL_S", "6"))
...
async def async_gpu_poll_loop():
    ...
    while True:
        try:
            ...
            output = await asyncio.to_thread(subprocess.check_output, cmd, timeout=1.5)
...
async def websocket_telemetry(websocket: WebSocket):
    ...
    try:
        # El broadcast loop alimenta esta conexión; aquí solo se espera la
        # desconexión del cliente (receive_text lanza WebSocketDisconnect).
        while True:
...
def system_info():
    ...
    return {
        ...
        "os": "Windows CUDA x86_64",
        "inference": CACHED_INFERENCE["available"],
        "model": str(MODEL_PATH),
        "engine": str(NANORTIME_EXE),
    }
