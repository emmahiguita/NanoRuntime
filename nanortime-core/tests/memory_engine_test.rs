...
use nanortime_core::memory_engine::{
    AdaptiveScheduler, CompressionLevel, DeviceClass, HardwareProfile, HardwareProfiler,
    KvAction, KvCacheOptimizer, MemoryPredictor, MemorySchedule, ModelProfile, NanoMemoryEngine,
    QualityPreserver, StorageManager, TokenImportance,
};
...
fn test_storage_manager_offload_roundtrip() {
    ...
    sm.offload_layer(&original_data, 10).expect("offload should succeed");
...
fn test_storage_manager_nonexistent_layer() {
    ...
    assert!(sm.load_layer(999).is_err(), "Loading non-offloaded layer should fail");
}
...
fn test_storage_manager_penalty_scales_with_size() {
    ...
    assert!(large_penalty > small_penalty, "Larger data should have larger penalty");
}
...
fn test_scheduler_strategy_adjusts() {
    let profile = test_profile(8192, 500.0, 8);
    let mut scheduler = AdaptiveScheduler::new(&profile, 32);
    assert_eq!(scheduler.current_strategy(), nanortime_core::memory_engine::SchedulingStrategy::Balanced);

    scheduler.adjust_strategy(5.0); // 5% quality drop
    assert_eq!(scheduler.current_strategy(), nanortime_core::memory_engine::SchedulingStrategy::Conservative);
}
...
fn test_predictor_identifies_consistent_hot_layers() {
    ...
    assert!(predicted.contains(&7), "Layer 7 should be consistently predicted");
}
...
fn test_predictor_no_hot_layers_when_uniform_low() {
    ...
    let predicted = predictor.predict(&scores, 1);
    assert!(predicted.is_empty(), "No layers should be predicted when all attention is low");
}
...
fn test_kv_optimizer_importance_ordering() {
    ...
    let tokens = vec![
        TokenImportance { token_id: 0, attention_score: 0.9, recency: 0.1, uniqueness: 0.9 }, // High
        TokenImportance { token_id: 1, attention_score: 0.1, recency: 0.1, uniqueness: 0.1 }, // Low
        TokenImportance { token_id: 2, attention_score: 0.5, recency: 0.5, uniqueness: 0.5 }, // Mid
    ];

    let actions = opt.optimize(&tokens, 30_000); // 30KB with tiny budget
    let evicted: Vec<usize> = actions.iter()
...
fn test_quality_preserver_perplexity_from_logits() {
    ...
    assert!(ppl < 3.0, "Confident logit should give low perplexity: {}", ppl);
}
