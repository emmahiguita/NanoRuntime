...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let req1 = UserRequest {
        prompt: "Explica la diferencia entre cuantización Q4_K_M y Q8_0 en GGUF para modelos LLM.".to_string(),
        context: None,
        history: None,
    };
    let resp1 = runtime.process_request(req1).await?;
    println!("  -> Tier usado: {}", resp1.tier_used);
    println!("  -> Confianza: {:.2}%", resp1.confidence.unwrap_or(0.0) * 100.0);
    println!("  -> Respuesta:\n{}", resp1.text);

    // Query 2: Consulta de Programación
    println!("\n[3/4] Ejecutando Consulta de Programación...");
    let req2 = UserRequest {
        prompt: "Escribe una función optimizada en Rust para calcular la entropía de Shannon.".to_string(),
        context: None,
        history: None,
    };
    let resp2 = runtime.process_request(req2).await?;
    println!("  -> Tier usado: {}", resp2.tier_used);
    println!("  -> Confianza: {:.2}%", resp2.confidence.unwrap_or(0.0) * 100.0);
    println!("  -> Respuesta:\n{}", resp2.text);
    ...
    println!("  -> Confianza: {:.2}%", resp3.confidence.unwrap_or(0.0) * 100.0);
    println!("  -> Respuesta:\n{}", resp3.text);
