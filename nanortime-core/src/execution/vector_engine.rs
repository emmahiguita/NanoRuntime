...
impl VectorEngine {
    pub async fn new(_config: &Config) -> Result<Self> {
        let persist_path = std::path::Path::new(&_config.memory.vector_db_path)
            .with_extension("json");
        let engine = Self {
    ...
    pub async fn index_document(
        &self, content: &str, metadata: serde_json::Value,
        embedding: Option<Vec<f32>>,
    ) -> Result<()> {
        ...
        // Auto-save to disk after indexing
    ...
    pub async fn search(
        ...
        Ok(scored.into_iter().take(top_k)
    ...
    pub async fn index_directory(&self, path: &str) -> Result<usize> {
        ...
        if !dir.exists() { tracing::warn!("Dir not found: {}", expanded_path); return Ok(0); }
        let exts = ["txt","md","rs","py","js","ts","json","toml","yaml","yml",
                    "html","css","java","kt","go","rb","php","c","cpp","h","hpp",
                    "sql","sh","ps1","bat","r","swift","dart","scala","lua"];
        const MAX_SIZE: u64 = 1 * 1024 * 1024; // 1MB

        // Collect file paths first (sync traversal)
        let mut files: Vec<std::path::PathBuf> = Vec::new();
        fn collect(dir: &Path, exts: &[&str], files: &mut Vec<std::path::PathBuf>) -> std::io::Result<()> {
            if dir.is_dir() {
                for e in std::fs::read_dir(dir)? {
                    let p = e?.path();
                    if p.is_dir() {
                        let n = p.file_name().unwrap_or_default().to_string_lossy();
                        if n.starts_with('.') || ["node_modules","target","build","dist",".git","vendor","cache"].contains(&n.as_ref()) { continue; }
        ...
        for fpath in &files {
            match tokio::fs::read_to_string(fpath).await {
                Ok(content) => {
                    ...
                    if trimmed.is_empty() || trimmed.len() < 20 { continue; }
        ...
        tracing::info!("Indexed {} files from {} (found {})", indexed, path, files.len());
        Ok(indexed)
    }

    pub async fn document_count(&self) -> usize { self.docs.read().await.len() }
    ...
    pub async fn embedding_dimension(&self) -> Option<usize> { *self.embed_dim.read().await }
    ...
    async fn load_from_disk(&self) {
        ...
        if let Some(parent) = path.parent() {
            if !parent.exists() {
                tracing::debug!("Vector engine directory {} does not exist, skipping load", parent.display());
                return;
            }
        }
    ...
    pub async fn import_json(&self, json: &str) -> Result<usize> {
        ...
        for entry in &entries {
            ...
            let embedding: Option<Vec<f32>> = entry["embedding"].as_array()
                .map(|a| a.iter().filter_map(|v| v.as_f64().map(|f| f as f32)).collect());
            if let Some(ref emb) = embedding {
                *self.embed_dim.write().await = Some(emb.len());
            }
            docs.push(IndexedDoc { content, metadata, embedding });
        }
        Ok(entries.len())
    }
}
...
pub fn cosine_similarity(a: &[f32], b: &[f32]) -> f32 {
    ...
    let d = (na * nb).sqrt();
    if d == 0.0 { 0.0 } else { (dot / d).clamp(0.0, 1.0) }
}
...
mod tests {
    ...
    async fn test_keyword_search() {
        ...
        e.index_document("NanoAI hybrid Rust", serde_json::json!({}), None).await.unwrap();
    ...
    async fn test_semantic_search() {
        ...
        clean_persist(&cfg);
        let e = VectorEngine::new(&cfg).await.unwrap();
        e.index_document("Rust language", serde_json::json!({"s":"r"}),
            Some(vec![1.0, 0.0, 0.0])).await.unwrap();
        e.index_document("Python language", serde_json::json!({"s":"p"}),
            Some(vec![0.0, 1.0, 0.0])).await.unwrap();
    ...
    async fn test_export_import() {
        ...
        clean_persist(&cfg);
        let e = VectorEngine::new(&cfg).await.unwrap();
        e.index_document("test", serde_json::json!({"k":"v"}), Some(vec![1.0,2.0])).await.unwrap();
    ...
    async fn test_clear() {
        ...
        clean_persist(&cfg);
        let e = VectorEngine::new(&cfg).await.unwrap();
        e.index_document("test", serde_json::json!({}), None).await.unwrap();
    ...
    async fn test_persistence() {
        ...
        let e = VectorEngine::new(&cfg).await.unwrap();
        e.index_document("persist test", serde_json::json!({"k":"v"}), None).await.unwrap();
        assert!(persist_path.exists(), "Persist file should exist after index");
