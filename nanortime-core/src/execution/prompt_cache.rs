...
impl PromptCache {
    ...
    pub fn get(&mut self, prompt: &str) -> Option<(String, Vec<f32>)> {
        ...
        let result = self.entries.get(&hash).map(|entry| {
            (entry.text.clone(), entry.probabilities.clone())
        });
        if result.is_some() {
    ...
    pub fn put(&mut self, prompt: &str, text: String, probabilities: Vec<f32>) {
        ...
        if self.entries.contains_key(&hash) {
            self.entries.insert(hash, CacheEntry { text, probabilities });
            self.move_to_front(hash);
            return;
        }
...
mod tests {
    ...
    fn test_cache_recently_used() {
        ...
        assert!(cache.get("A").is_some(), "A was recently used, should survive");
