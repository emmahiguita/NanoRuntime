...
impl KvCacheOptimizer {
    ...
    pub fn optimize(
        ...
        if current_size <= self.target_size {
            return tokens.iter().map(|t| (t.token_id, KvAction::Keep)).collect();
        }

        let n_tokens = tokens.len();
        let bytes_to_free = current_size.saturating_sub(self.target_size);
        let bytes_per_token = if n_tokens > 0 { current_size / n_tokens } else { 0 };

        // Calcular importancia de cada token
        let mut ranked: Vec<(usize, f32, bool)> = tokens.iter().enumerate()
            .map(|(idx, t)| {
        ...
        for (ranked_idx, &(token_id, importance, is_recent)) in ranked.iter().rev().enumerate() {
            let token_idx = tokens.iter().position(|t| t.token_id == token_id).unwrap_or(ranked_idx);
            ...
            if is_recent {
                ...
                if freed_bytes < bytes_to_free && importance < self.eviction_threshold * 0.5 {
                    ...
                    freed_bytes += bytes_per_token / CompressionLevel::Int8.compression_ratio() as usize;
                }
                continue;
            }
            ...
            if importance < self.eviction_threshold {
                ...
                let saved = bytes_per_token
                    - (bytes_per_token as f32 / self.compression_level.compression_ratio()) as usize;
                freed_bytes += saved;
            }
    ...
    pub fn estimate_savings(
        actions: &[(usize, KvAction)],
        bytes_per_token: usize,
    ) -> usize {
        actions.iter().map(|(_, action)| {
            match action {
                KvAction::Keep => 0,
                KvAction::Evict => bytes_per_token,
                KvAction::Compress(level) => {
                    let compressed_size = (bytes_per_token as f32 / level.compression_ratio()) as usize;
...
mod tests {
    ...
    fn test_calculate_importance_zero_attention() {
        ...
        let score = KvCacheOptimizer::calculate_importance(&t);
        assert!(score > 0.0, "Score should be > 0 even with zero attention (min floor)");
    }
    ...
    fn test_optimize_evicts_low_importance() {
        ...
        let tokens: Vec<TokenImportance> = (0..20).map(|i| {
        ...
        let evicted = actions.iter().filter(|(_, a)| *a == KvAction::Evict).count();
    ...
    fn test_optimize_protects_recent_tokens() {
        ...
        for i in 4..20 { // Last 16 tokens (index 4-19)
            let action = &actions.iter().find(|(id, _)| *id == i).unwrap().1;
            assert_ne!(action, &KvAction::Evict, "Recent token {} should not be evicted", i);
        }
    }
