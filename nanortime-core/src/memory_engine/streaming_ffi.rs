...
use crate::memory_engine::cache_aware_loader::{CacheAwareLoader, StreamingConfig, can_stream_model};
...
#[no_mangle]
pub extern "C" fn nanortime_streaming_init(
    gguf_path: *const std::os::raw::c_char,
    window_layers: std::os::raw::c_int,
) -> std::os::raw::c_int {
    let path_str = unsafe {
        if gguf_path.is_null() { return -1; }
        std::ffi::CStr::from_ptr(gguf_path).to_string_lossy().into_owned()
    };
    ...
    // Collect layer ranges sorted by layer index
    ...
    let loader = match CacheAwareLoader::new(
        &file,
        &ranges,
        config,
    ) {
    ...
    *LAYER_COUNT.lock().unwrap() = total;
    *LOADER.lock().unwrap() = Some(loader);
...
#[no_mangle]
pub extern "C" fn nanortime_streaming_load(
    ...
    match loader.load_layer(layer_idx as usize) {
        Ok(_) => {
            unsafe { loader.get_layer_ptr(layer_idx as usize) }
...
#[no_mangle]
pub extern "C" fn nanortime_streaming_release(
    _layer_idx: std::os::raw::c_int,
) {
    let mut guard = LOADER.lock().unwrap();
...
pub extern "C" fn nanortime_streaming_can_run(
    ...
    let bytes_per_layer = 140 * 1024 * 1024; // ~140 MB per layer for 7B Q4_K_M
    if can_stream_model(
...
pub extern "C" fn nanortime_streaming_layer_count() -> std::os::raw::c_int {
    *LAYER_COUNT.lock().unwrap() as std::os::raw::c_int
}
