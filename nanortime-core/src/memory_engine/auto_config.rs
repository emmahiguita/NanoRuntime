    ...
    pub fn from_profile(profile: DeviceProfile) -> Self {
        ...
        let (max_ctx, batch, kv_comp, page_strat, speculative, threads) = match tier {
            DeviceTier::Budget => {
                ...
                (ctx, 256, KvCompression::Int4, PageStrategy::Conservative, false, 2)
            }
            DeviceTier::MidRange => {
                // OPPO CPH2557: ~4 GB usable. Full context.
                (8192, 512, KvCompression::None, PageStrategy::Balanced, false, 4)
            }
            DeviceTier::Flagship => {
                (8192, 512, KvCompression::None, PageStrategy::Aggressive, true, profile.cpu_cores as usize / 2)
            }
            DeviceTier::Desktop => {
                (8192, 1024, KvCompression::None, PageStrategy::Aggressive, true, profile.cpu_cores as usize)
            }
        };
