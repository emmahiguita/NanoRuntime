...
impl ExecutionPlanner {
    ...
    pub fn plan_boot(&self, profile: &DeviceProfile) -> PlanResult {
        ...
        let use_streaming = risk == "Critical"
            && self.model.can_stream(2, self.bytes_per_layer_mb, self.num_layers, total);
        ...
        let vma = if use_streaming {
            self.model.estimate_streaming_vma(
                window,
                self.bytes_per_layer_mb,
                self.num_layers,
            )
        } else {
            self.model_size_mb
    ...
    pub fn plan_survival(&self, available_mb: f64, total_mb: f64) -> PlanResult {
        ...
        let use_streaming = self.model_size_mb > total_mb
            && self.model.can_stream(1, self.bytes_per_layer_mb, self.num_layers, total_mb);
        let window = if use_streaming { 1 } else { 0 };

        let vma = if use_streaming {
            self.model.estimate_streaming_vma(window, self.bytes_per_layer_mb, self.num_layers)
        } else {
            self.model_size_mb
        };
        ...
        let config = RuntimeConfig {
            ...
            min_context: 256,
            tier: DeviceProfile::default().tier,
            profile: DeviceProfile::default(),
        };

        let summary = format!(
            "plan_survival: ratio={:.0}% ctx={} kv={:?} streaming={} vma={:.0}MB",
            ratio * 100.0, ctx, kv, use_streaming, vma
        );
    ...
    pub fn plan_cautious(&self, available_mb: f64) -> PlanResult {
...
mod tests {
    ...
    fn test_plan_survival_triggers_streaming() {
        let planner = ExecutionPlanner::new(4470.0, 32);
        let plan = planner.plan_survival(300.0, 3814.0);
    ...
    fn test_plan_cautious_reduces_context() {
        let planner = ExecutionPlanner::new(4470.0, 32);
        let plan = planner.plan_cautious(1200.0);
