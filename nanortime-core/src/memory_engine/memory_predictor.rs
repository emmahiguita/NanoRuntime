...
impl MemoryPredictor {
    ...
    pub fn predict(&mut self, attention_scores: &[f32], lookahead: usize) -> Vec<usize> {
        ...
        let mut hot_layers: Vec<usize> = avg_scores.iter().enumerate()
    ...
    pub fn current_pattern(&self) -> Option<AttentionPattern> {
        ...
        let hot_layers = latest.iter().enumerate()
    ...
    fn detect_trending_layers(&self, n_layers: usize) -> Vec<usize> {
        ...
        for layer_id in 0..n_layers {
            let s0 = recent.get(2).and_then(|v| v.get(layer_id)).copied().unwrap_or(0.0);
            let s1 = recent.get(1).and_then(|v| v.get(layer_id)).copied().unwrap_or(0.0);
            let s2 = recent.first().and_then(|v| v.get(layer_id)).copied().unwrap_or(0.0);
...
mod tests {
    ...
    fn test_predict_with_lookahead_expands_neighbors() {
        ...
        assert!(predicted.contains(&4) || predicted.contains(&6),
    ...
    fn test_trending_detection() {
        ...
        assert!(trending.contains(&2), "Rapidly increasing layer should be detected as trending");
    }
