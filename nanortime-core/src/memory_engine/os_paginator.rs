...
// OSMemoryPaginator puede compartirse entre hilos
...
impl OSMemoryPaginator {
    ...
    pub fn prefetch_range(&self, range: &ByteRange) -> Result<(), std::io::Error> {
        ...
        #[cfg(windows)]
        {
            ...
            if res == 0 { // 0 indicates failure in PrefetchVirtualMemory
                return Err(std::io::Error::last_os_error());
            }
        }
    ...
    pub fn set_access_pattern(&self, range: &ByteRange, pattern: AccessPattern) -> Result<(), std::io::Error> {
        let (ptr, len) = self.get_aligned_ptr_and_len(range)?;
        if len == 0 {
            return Ok(());
        }
    ...
    fn get_aligned_ptr_and_len(&self, range: &ByteRange) -> Result<(*mut c_void, usize), std::io::Error> {
...
mod tests {
    ...
    fn test_aligned_ptr_and_len() {
        ...
        let range = ByteRange { start: 5000, end: 15000 };
