...
impl HallucinationDetector {
    ...
    /// Crea un detector con valores por defecto razonables.
    pub fn default() -> Self {
    ...
    pub fn feed_token(&mut self, token: &str, probability: f32) {
        ...
        if probability > 0.0 {
            let ppl = (-probability.ln()).max(0.0).min(100.0);
...
mod tests {
    ...
    fn test_no_hallucination_normal_tokens() {
        ...
        let tokens = vec![("fn", 0.95), (" ", 0.90), ("main", 0.92), ("(", 0.88), (")", 0.85)];
        for (t, p) in &tokens { d.feed_token(t, *p); }
    ...
    fn test_detects_low_probability() {
        ...
        assert_eq!(signal.as_ref().unwrap().htype, HallucinationType::LowProbability);
    ...
    fn test_detects_repetition() {
        ...
        for t in &reps { d.feed_token(t, 0.5); }
        let signal = d.check();
        assert!(signal.is_some(), "Repetition should trigger");
        assert_eq!(signal.as_ref().unwrap().htype, HallucinationType::Repetition);
    }

    #[test]
    fn test_high_perplexity_detection() {
        let mut d = HallucinationDetector::new(0.2, 0.8, 5);
        for _ in 0..10 { d.feed_token("xyz", 0.01); }
        let signal = d.check();
    ...
    fn test_reset() {
        let mut d = HallucinationDetector::new(0.2, 0.8, 5);
        for _ in 0..7 { d.feed_token("a", 0.01); }
