...
impl ServerState {
    ...
    fn run_inference(&self, prompt: &str, max_tokens: usize) -> ChatResponse {
        ...
        match result {
            Ok(single) => {
                *self.last_tok_s.lock().unwrap() = single.tok_s;
                *self.last_confidence.lock().unwrap() = single.confidence;
                ChatResponse {
...
fn main() {
    ...
    println!("  Desde el móvil (misma WiFi): http://IP_DE_ESTE_PC:{}", port);
    println!("");

    let state = Arc::new(ServerState::new(&model));
    let listener = TcpListener::bind(("0.0.0.0", port)).expect("No se pudo bindear el puerto");
